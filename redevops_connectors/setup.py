...
SETUP_GUIDES: Dict[str, ProviderSetupGuide] = {
    ...
    "whatsapp_waha": ProviderSetupGuide(
        provider="whatsapp_waha", display_name="WhatsApp (WAHA, self-hosted)", auth_type=AuthType.API_KEY,
        used_for="Send/receive WhatsApp on multiple numbers via a self-hosted WAHA gateway (no Meta approval).",
        setup_url="https://waha.devlike.pro/",
        credential_fields=(
            _key("WHATSAPP_API_KEY", label="WAHA API key (X-Api-Key)", example="…"),
            CredentialField(name="base_url", label="WAHA base URL", secret=False,
                            env_var="WAHA_BASE_URL", example="http://localhost:3000"),
            CredentialField(name="session", label="Session name", secret=False,
                            env_var="WAHA_SESSION", example="default"),
        ),
        account_prerequisites=("Run a WAHA instance and pair a number by scanning the QR (session → WORKING).",),
        manual_steps=("Deploy WAHA (docker run devlikeapro/waha); set WHATSAPP_API_KEY on it.",
                      "Start a session and scan the QR from the WhatsApp app to pair the number.",
                      "Point base_url at your WAHA instance; a session reaches status WORKING when connected."),
